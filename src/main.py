...
#from SpellChecker import correct_sentence, correct_sentence_spellchecker, correct_sentence_textblob
...
def train(model, loader):
    ...
    earlyStopping = 75  # Stop training after this number of epochs without improvement
    batchNum = 0
    print("Length of loader.trainSamples:",len(loader.trainSamples))
...
def load_different_image():
    ...
    for i in range(1, Model.batchSize):
       imgs.append(preprocessor(cv2.imread("../data/sample_images/a ({}).png".format(i), cv2.IMREAD_GRAYSCALE), Model.imgSize, enhance=False))
    return imgs
...
def main():
    ...
    parser.add_argument(
        "--infer", type =str, help="path to infer a single image")
